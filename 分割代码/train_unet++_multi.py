...
def f_score(pr, gt, beta=1, eps=1e-7, threshold=None, ignore_channels=None):
    ...
    # 添加权重项
    weights = torch.tensor([5/12, 2/3, 3/5, 3/2]).to(pr.device)
...
if __name__ == '__main__':
    ...
    with open('training_logs_unet++.txt', 'w') as f:# open和write是自己加的，w表示先删除原有的内容再写
        for i in range(0, 100):
            with open('480_training_logs_unet++.txt', 'a') as f:# open和write是自己加的，w表示先删除原有的内容再写
                ...
                if min_dice_loss > valid_logs['dice_loss']:
                    ...
                    torch.save(model, './480_best_model_unet++.pth')
                    print('Model saved!')
                    f.write('Model saved!\n\n'.format(i))
            ...
            plt.savefig('480_loss_plot_unet++.png')  # 保存为 loss_plot.png 文件
            plt.show()
            ...
            plt.savefig('480_iou_plot_unet++.png')  # 保存为 iou_plot.png 文件
            plt.show()