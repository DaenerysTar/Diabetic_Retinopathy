...
if __name__ == '__main__':
    ...
    with open('training_logs_unet++.txt', 'w') as f:# open和write是自己加的，w表示先删除原有的内容再写
        for i in range(0, 100):
            with open('training_logs_manet.txt', 'a') as f:# open和write是自己加的，w表示先删除原有的内容再写
                print('\nEpoch: {}'.format(i))
            ...
            if min_dice_loss > valid_logs['dice_loss']:
                ...
                torch.save(model, './best_model_manet.pth')
                print('Model saved!')
            ...
            plt.savefig('loss_plot_manet.png')  # 保存为 loss_plot.png 文件
            plt.show()
            ...
            plt.savefig('iou_plot_manet.png')  # 保存为 iou_plot.png 文件
            plt.show()