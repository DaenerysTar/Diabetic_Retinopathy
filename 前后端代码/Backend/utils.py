...
def train(model, train_loader, val_loader, criterion, optimizer, n_epochs, device):
    ...
    for epoch in range(n_epochs):
        ...
        print('Train |Epoch %i: loss = %f | accuracy: %f' % (epoch, train_metrics['mean_loss'], train_metrics['accuracy']))
        print('Validation |Epoch %i: loss = %f | accuracy: %f' % (epoch, val_metrics['mean_loss'], val_metrics['accuracy']))
        print('\n')
    ...
    plot_metrics(history_train, history_val)
...
def test(model, data_loader, criterion, device):
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader, 0):
            ...
            bs = labels.size()[0]
            num_classes = 5
            onehot = (labels.reshape(bs, 1) == torch.arange(num_classes,device=labels.device).reshape(1, num_classes)).float()
            ###
...
def compute_metrics(scores, ground_truth, onehot, prediction):
    ...
    onehot = np.array([x.cpu().detach().numpy() for x in onehot])
    scores = np.array([x.cpu().detach().numpy() for x in scores])
